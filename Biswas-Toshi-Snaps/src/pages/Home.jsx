...
const Home = () => {
  ...
  useEffect(() =>{
    async function register(){
      ...
      if (typeof id !== "string"){
        const paragraph = await axios.get("https://unit-3-project-c5faaab51857.herokuapp.com/"+"register");
        id = paragraph.data.api_key;
    ...
    async function getPhoto(){
      const response = await axios.get("https://unit-3-project-c5faaab51857.herokuapp.com/"+"photos/"+"?api_key="+String(token));
            ...
            <div className="content-left"style={{width: "100%"}}>
              <div className="missions">
                ...
                <h1 className="mission_quote">Provide photographers a space to share
                    photos of the neighborhoods they cherish, 
                    <span className="mission_quote-italics">
                        expressed in their unique style.
                    </span>
                </h1>
              </div>
              <Content items={filteredItems} data = {token}/>   
            </div>
          </div>
          <Footer/>
        </div>
